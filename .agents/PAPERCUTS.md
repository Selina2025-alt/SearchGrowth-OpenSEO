...
- [ ] `2026-09-14T04:35:00Z` — `claude` — A fresh task worktree gets no `node_modules`, but the task's APPROVED COMMANDS list omits any install step, so the first approved gate (`corepack pnpm exec vitest` / `prettier`, then `format:check`, `types:check`, `lint`, `test`, `build`, `ci:check`) dies with a bare "command not found" until `corepack pnpm install --frozen-lockfile` is run. Add the frozen install to the worktree bootstrap (or to the approved-command list) so executors don't have to step outside their approved commands before any gate can run.
- [ ] `2026-09-05T09:36:00Z` — `codex` — `.ai-orchestrator/probe-environment.ps1` names its captured argument parameter `$Args`, colliding with PowerShell's automatic variable; version probes invoke CLIs without `--version` and report misleading interactive/input errors. Rename the parameter and splat a distinct argument array; the separate model probe still succeeds.
